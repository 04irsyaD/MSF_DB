...
- **`.gitignore` sempat menelan berkas template.** Aturan `*.docx` membuat template yang di-commit ke `backend/templates/` diabaikan git tanpa satu pun pesan error, sehingga tidak pernah masuk repositori maupun sampai ke server. Prinsip "template hanya dari repositori" pada spec section 9.1 karena itu runtuh secara diam-diam. Ditutup dengan negasi `!backend/templates/*.docx` beserta test yang menjaganya.
...
- **Keluaran DOCX berbentuk Technical Specification Document lewat `structure_template=msf_tsd`.** Template Word aslinya yang diisi memakai `docxtpl`, bukan ditiru dengan kode, sehingga sampul, kop, header, footer, penomoran, dan seluruh gaya bertahan tanpa pemeliharaan visual. Template disiapkan lewat `scripts/siapkan_template_tsd.py`, yang menyusutkan dokumen sumber 7,7 MB menjadi kerangka 1,7 MB. Jinja2 dijalankan di dalam sandbox dan nama berkas template dipetakan konstan dari Enum, sehingga nilai dari request tidak pernah menyentuh path. Tidak ada endpoint upload template, dan tidak boleh ada. Lihat [ADR-006](decisions/006-template-docx-docxtpl.md).
- **Rewrite API mode development diarahkan ke port backend v2.** `frontend/next.config.js` menunjuk `http://localhost:8080`, yaitu port msf-app v1, sehingga frontend v2 dalam mode development berbicara ke backend v1 dan seluruh aplikasi menampilkan layar pemeliharaan. Tidak pernah ketahuan karena v2 selalu dijalankan lewat Docker dalam mode produksi. Kini memakai `BACKEND_ORIGIN` dengan bawaan `http://localhost:8001`.
- **Selector Struktur Dokumen di frontend.** `GeneratePanel` menampilkan pilihan Standard dan MSF TSD mengikuti pola selector Detail Level yang sudah ada, dan `structure_template` diteruskan pada kedua payload generate. Peringatan tampil bila MSF TSD dipilih bersama format selain DOCX, karena template hanya berlaku untuk DOCX.
- **`MAX_OUTPUT_SIZE_MB` dihidupkan.** Sebelumnya dideklarasikan di `.env.example` tetapi tidak dibaca kode mana pun. Template TSD memperbesar keluaran secara berarti, sedangkan berkas hasil dikirim utuh dalam satu respons.
- **Prompt menyatakan bahasa keluaran secara eksplisit.** Prompt berbahasa Indonesia saja ternyata tidak cukup: pada uji dengan model sungguhan, ringkasan tabel keluar Bahasa Indonesia sementara seluruh baris per kolom keluar Bahasa Inggris. Instruksi bahasa sempat hilang ketika kontrak keluaran AI diganti.
