...
# Load .env from the summary folder explicitly (backup may be executed from repo root)
# Try to load the .env from the original summary folder (backup may run from different cwd)
env_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '..', 'summary', '.env'))
...
def main():
    ...
    for i, (table_name, cols) in enumerate(tables_to_process.items(), 1):
        ...
        # db_reader may return a dict with keys like 'columns', 'primary_keys'.
        # Support both shapes: either a list of column dicts, or a dict containing 'columns'.
